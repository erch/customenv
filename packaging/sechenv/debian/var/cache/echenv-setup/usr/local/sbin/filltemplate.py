...
def usage():
    print "usage : fill-template -e|--env-input <env file> -j|--json-input <JSON file> -y|--yaml-input <YAML file> -t|--template <template file> [-o|--output <output file>]"
    ...
    print "   -y|--yaml-input <YAML file>       the path to the YAML file"
...
def parse_options(argv):
    if len(argv) == 0:
        ...
        sys.exit(2)
    try:
        opts, args = getopt.getopt(argv, "he:j:y:t:o:", ["help", "env-input=", "json-input=", "yaml-input=", "template=", "output="])
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            json_input = arg
        elif opt in ("-y", "--yaml-input"):
            yaml_input = arg
        elif opt in ("-t", "--template"):
            template = arg
        elif opt in ("-o", "--output"):
            output = arg
    return template, output, env_input, json_input, yaml_input
...
def main(argv):
    template, output, env_input, json_input, yaml_input = parse_options(argv)
    decoded = []
    if env_input == None and json_input == None and yaml_input == None:
        print "ERROR: Specify at least at one input."
        sys.exit()
    elif env_input != None and json_input != None and yaml_input != None:
        ...
        for i in json_input.split(","):
            ...
            f.close()
    elif yaml_input != None:
        for i in yaml_input.split(","):
            f = open(i)
            decoded.append(yaml.load(f))
            f.close()
    template_input = open(template, 'r').read()
